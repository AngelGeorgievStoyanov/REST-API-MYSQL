...
const updateSql = `UPDATE hack_trip.trips SET title =?, description=?, price=?, transport=?, countPeoples=?, typeOfPeople=?, destination=?, imageUrl=?, lat=?,lng=?, timeEdited=? WHERE _id =?`;
...
export class TripRepository implements ITripRepository<Trip> {
    ...
    async create(trip: Trip): Promise<Trip> {
        ...
        return new Promise((resolve, reject) => {
            this.pool.query(createSql,
                [trip.title, trip.description, trip.price, trip.transport, trip.countPeoples, trip.typeOfPeople, trip.destination, trip.imageUrl, trip.coments, trip.likes, trip._ownerId, trip.lat, trip.lng, trip.timeCreated, trip.timeEdited, trip.reportTrip],
                (err, rows, fields) => {
                    if (err) {
    ...
    async updateTripById(id: IdType, trip: Trip): Promise<Trip> {
        ...
        return new Promise((resolve, reject) => {
            ...
            trip.countPeoples, trip.typeOfPeople, trip.destination, trip.imageUrl, trip.lat, trip.lng, trip.timeEdited, id], (err, rows, fields) => {
                if (err) {
